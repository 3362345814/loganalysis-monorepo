...
当前文档对应版本：`v0.5.1`（发布于 `2026-04-22`）
...
```bash
curl -fsSL "https://raw.githubusercontent.com/3362345814/loganalysis-monorepo/v0.5.1/scripts/install.sh" | sh
```

#### Windows PowerShell

```powershell
irm "https://raw.githubusercontent.com/3362345814/loganalysis-monorepo/v0.5.1/scripts/install.ps1" | iex
```
...
curl.exe -L --fail -o $bin "https://github.com/3362345814/loganalysis-monorepo/releases/download/v0.5.1/loganalysis-windows-amd64.exe"
& $bin version
& $bin doctor
```
...
```bash
curl -fL -o /tmp/loganalysis "https://github.com/3362345814/loganalysis-monorepo/releases/download/v0.5.1/loganalysis-linux-amd64"
...
# 升级到指定版本
loganalysis upgrade --to v0.5.1

# 允许主版本升级
loganalysis upgrade --to v2.0.0 --allow-major

# 若你希望严格按固定端口升级（不自动避让）
loganalysis upgrade --to v0.5.1 --no-auto-port
```
