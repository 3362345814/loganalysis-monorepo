...
当前文档对应版本：`v1.2.10`（发布于 `2026-04-29`）
...
```bash
curl -fsSL "https://raw.githubusercontent.com/3362345814/loganalysis-monorepo/v1.2.10/scripts/install.sh" | sh
```

#### Windows PowerShell

```powershell
irm "https://raw.githubusercontent.com/3362345814/loganalysis-monorepo/v1.2.10/scripts/install.ps1" | iex
```
...
curl.exe -L --fail -o $bin "https://github.com/3362345814/loganalysis-monorepo/releases/download/v1.2.10/loganalysis-windows-amd64.exe"
& $bin version
& $bin doctor
```
...
```bash
curl -fL -o /tmp/loganalysis "https://github.com/3362345814/loganalysis-monorepo/releases/download/v1.2.10/loganalysis-linux-amd64"
...
loganalysis up --version v1.2.10
loganalysis status
```
...
# 升级到指定版本
loganalysis upgrade --to v1.2.10
...
# 若你希望严格按固定端口升级（不自动避让）
loganalysis upgrade --to v1.2.10 --no-auto-port
```
