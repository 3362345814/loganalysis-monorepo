...
当前文档对应版本：`v0.2.4`（发布于 `2026-04-12`）
...
```bash
curl -fsSL "https://raw.githubusercontent.com/3362345814/loganalysis-monorepo/v0.2.4/scripts/install.sh" | sh
```

#### Windows PowerShell

```powershell
irm "https://raw.githubusercontent.com/3362345814/loganalysis-monorepo/v0.2.4/scripts/install.ps1" | iex
```
...
curl.exe -L --fail -o $bin "https://github.com/3362345814/loganalysis-monorepo/releases/download/v0.2.4/loganalysis-windows-amd64.exe"
& $bin version
& $bin doctor
```
...
```bash
curl -fL -o /tmp/loganalysis "https://github.com/3362345814/loganalysis-monorepo/releases/download/v0.2.4/loganalysis-linux-amd64"
...
```bash
loganalysis doctor
loganalysis up --profile minimal --version v0.2.4 --auto-port
loganalysis status
```
...
# 手动指定版本 + 自动避让端口
loganalysis up -version v0.2.4 --auto-port
```
...
```bash
loganalysis up --profile db --version v0.2.4
